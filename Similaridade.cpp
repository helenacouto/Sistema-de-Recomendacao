...
void comparaMemoria (Similaridade *sim) {
    long memoriaDensa = (long) sim->n * sim->n * sizeof (double);

    long memoriaCSR = sim->intersecaoCSR.values.size() * sizeof(int) +
                      sim->intersecaoCSR.col_index.size() * sizeof(int) +
                      sim->intersecaoCSR.row_ptr.size() * sizeof(int);

    printf("Memoria estimada ocupada pela matriz de intersecao (densa): %.2f MB\n", memoriaDensa);
    printf("Memoria estimada ocupada pela matriz de intersecao (CSR): %.2f MB\n", memoriaCSR);
}

void testadorComparaSimilaridade (ListaCompras *lista) {
    Similaridade simDenso, simCSR;

    if (!criaMatrizSimilaridade(lista, &simDenso, 1)) return 0;
    printf("Abordagem densa (eficiente): %.6f segundos\n", simDenso.tempo);

    if (!criaMatrizSimilaridadeCSR(lista, &simCSR)) return 0;
    printf("Abordagem com estrutura CSR: %.6f segundos\n", simCSR.tempo);

    printf("Comparando dois clientes com os dois métodos: \n");
    for (int i = 0; i < 3 && i < simCSR.n; i++) {
        testadorExibeSimilaridade(&simDenso, lista, i);
        testadorExibeSimilaridadeCSR(&simCSR, lista, i);
    }
