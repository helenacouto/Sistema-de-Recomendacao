...
void ordenaListaCompras (ListaCompras *lista, Similaridade *sim) {
    for (int i = 0; i < sim->n; i++) {
        ...
        for (int produto : produtosCliente) {
            sim->vetorCompras.push_back(produto);
        }
    }
}
...
int criaMatrizComprasCSR(ListaCompras *lista, Similaridade *sim, MatrizCSR *matrizDestino, vector<int> &vetorCompras) {
    ...
    for (int i = 0; i < sim->n; i++) {
        ...
        for (int k = 0; k < qtdProdutosCliente; k++) {
            int produto = sim->vetorCompras[cont];
...
int criaIntersecaoCSR(MatrizCSR *matrizCompras, MatrizCSR *matrizDestino, Similaridade *sim, vector<int> &vetorCompras) {
    matrizDestino->n_linhas = sim->n;
...
int criaMatrizSimilaridadeCSR (ListaCompras *lista, Similaridade *sim) {
    ...
    if (!criaIntersecaoCSR(&sim->comprasCSR; &sim->intersecaoCSR, sim, vetorCompras)) return 0;
